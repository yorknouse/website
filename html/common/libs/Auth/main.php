    ...
    function __construct()
    {
        ...
        if (isset($_SESSION['token'])) {
            ...
            if ($tokencheckresult != null) {
                if ((strtotime($tokencheckresult["authTokens_created"]) + (3 * 24 * (3600))) < time() or $tokencheckresult["authTokens_ipAddress"] != (isset($_SERVER["HTTP_CF_CONNECTING_IP"]) ? $_SERVER["HTTP_CF_CONNECTING_IP"] : $_SERVER["REMOTE_ADDR"])) {
                    $this->login = false;
