...
$DBLIB->where("(articlesDrafts.articlesDrafts_byline LIKE '%" . $term . "%' OR articlesDrafts.articlesDrafts_headline LIKE '%" . $term . "%')");
...
$articles = $DBLIB->get("articles", 3, ["articles.articles_id", "articles.articles_published","articles.articles_slug","articlesDrafts.articlesDrafts_headline","articlesDrafts.articlesDrafts_byline"]);
if (!$articles) finish(true, null, null);
$output = [];
foreach ($articles as $article) {
    $article['url'] = $CONFIG['ROOTURL'] . '/' . date("Y/m/d", strtotime($article['articles_published'])) . "/" . $article['articles_slug'];
    $output[] = $article;
}
