...
if ($_POST['users_userid'] != $USERDATA['users_userid'] && $AUTH->permissionCheck(5)) $userid = $bCMS->sanitiseString($_POST['users_userid']);
else $userid = $USERDATA['users_userid'];

$DBLIB->where("users_userid", $userid);
if ($DBLIB->update('users', ["articles_featured" < implode(",", explode(",", $bCMS->sanitiseString($_POST['featured'])))]))
