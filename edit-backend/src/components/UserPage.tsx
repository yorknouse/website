...
function updateThumbnailFunc(id: number, setThumbnailURL: Dispatch<SetStateAction<string | null>>) {
    return async () => {
        const res = await fetch(`/api/backend/users/${id}`);
        const updatedUser = await res.json();
        if (updatedUser?.users_thumbnail) {
            const newURLRes = await fetch(`/api/s3url?size=false&fileId=${updatedUser.users_thumbnail}`);
