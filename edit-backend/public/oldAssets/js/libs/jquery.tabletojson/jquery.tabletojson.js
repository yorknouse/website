(function ($) {
  ...
  $.fn.tableToJSON = function (opts) {
    ...
    var cellValues = function (cellIndex, cell, isHeader) {
      ...
      return $.trim(
        override ||
          (opts.allowHTML ? $cell.html() : cell.textContent || $cell.text()) ||
          "",
      );
    };
    ...
    var rowValues = function (row, isHeader) {
      ...
      var useRowId =
        typeof includeRowId === "boolean"
          ? includeRowId
          : typeof includeRowId === "string"
            ? true
            : false;
      var rowIdName =
        (typeof includeRowId === "string") === true ? includeRowId : "rowId";
      if (useRowId) {
      ...
      table
        ...
        .each(function (rowIndex, row) {
          if (rowIndex > 0 || notNull(opts.headings)) {
            ...
            var isEmpty =
              $row.find("td").length === $row.find("td:empty").length
                ? true
                : false;

            if (
              ($row.is(":visible") || !opts.ignoreHiddenRows) &&
              (!isEmpty || !opts.ignoreEmptyRows) &&
              (!$row.data("ignore") || $row.data("ignore") === "false")
            ) {
              cellIndex = 0;
              ...
              $row.children().each(function () {
                ...
                txt =
                  tmpArray[rowIndex][cellIndex] || cellValues(cellIndex, $cell);
                if (notNull(txt)) {
