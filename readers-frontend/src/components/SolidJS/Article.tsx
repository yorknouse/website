...
const Article: Component<ArticleProps> = (props) => {
  const getBorderColour = (): string => {
    if (props.isInMuseComponent) return "border-transparent";
  ...
  const getTextColour = (): string => {
    if (props.isInMuseComponent) return "text-white";
    switch (props.categoryColor?.toUpperCase()) {
      case "#E4362D":
          ...
          {!props.isVertical && (
            <div class="flex">
              <a class="h-full w-1/2" href={props.articleUrl}>
                <img
                  class="aspect-{9/16} rounded-xl object-cover "
                  src={props.imageUrl}
                  alt="Article Thumbnail"
                />
              </a>
              <div class={`${getBorderColour()} ml-3 h-full w-1/2 border-t-2`}>
                {!props.isInMuseComponent && (
                ...
                <div class="mb-2">
                  <a href={props.articleUrl}>
                    <p
                      class={`text-l xl:text-xl 2xl:text-2xl ${getContentColour()}`}
                    >
                      {props.headline}
                    </p>
                  </a>
                  <a href={`/author/${props.authorId}`}>
                    <p
                      class={`text-sm italic 2xl:text-base ${getContentColour()}`}
                    >
                ...
                <a href={props.articleUrl}>
                  <p
                    class={`text-xs xl:text-sm 2xl:text-base ${getContentColour()}`}
                  >
                    {props.excerpt}
                  </p>
                </a>
              ...
              <a class="w-full" href={props.articleUrl}>
                <img
                  class={`mb-5 aspect-video rounded-xl object-cover ${
                    props.isInMuseComponent && "min-h-[200px]"
                  }`}
                  src={props.imageUrl}
                  alt="Article Thumbnail"
                />
